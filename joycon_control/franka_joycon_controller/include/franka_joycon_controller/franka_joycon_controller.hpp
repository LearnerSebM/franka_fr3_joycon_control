...
/**
 * Joint impedance controller with IK that receives desired pose from joycon command.
 * Uses inverse kinematics (LMA) from Orocos KDL to convert desired pose to joint positions.
...
class FrankaJoyconController : public controller_interface::ControllerInterface {
 ...
 private:
  ...
  /**
   * @brief Helper function to convert roll, pitch, yaw to quaternion
   */
